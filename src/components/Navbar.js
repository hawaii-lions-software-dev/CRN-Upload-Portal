...
function Navbar() {
  ...
  return (
    <div>
        ...
        <Container maxWidth="xl">
          <Toolbar disableGutters>
            ...
            {currentUser && <Box sx={{ flexGrow: 1, display: { xs: 'flex', md: 'none' } }}>
              ...
              >
                ...
                <MenuItem key='logout' onClick={handleCloseUserMenu}>
                  <Typography textAlign="center">
                    <Button onClick={handleLogout} style={{textDecoration: "none", color: "black"}}>Log out</Button>
                  </Typography>
                </MenuItem>
              </Menu>
            </Box>}
            ...
            {currentUser &&<Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }}>
              ...
              <MenuItem key='logout' onClick={handleCloseUserMenu}>
                <Typography textAlign="center">
                  <Button onClick={handleLogout} style={{textDecoration: "none", color: "white"}}>Log out</Button>
                </Typography>
              </MenuItem>
            </Box>}
            ...
            {currentUser && <Box sx={{ flexGrow: 0 }}>
              <Tooltip title="Open settings">
                <Button onClick={handleOpenUserMenu} sx={{ p: 0 }} style={{textDecoration: "none", color: "white"}}>
                  My Account
                </Button>
              </Tooltip>
              <Menu
              ...
              >
                {settings.map((setting) => (
                  <MenuItem key={setting} onClick={handleCloseUserMenu}>
                    <Typography textAlign="center">
                      <Link to={`/${setting}`} style={{textDecoration: "none", color: "black"}}>{setting}</Link>
                    </Typography>
                  </MenuItem>
                ))}
                <MenuItem key='logout' onClick={handleCloseUserMenu}>
      ...
      {error && <Alert variant="danger" style={{marginBottom: '35px'}}>{error}</Alert>}
    </div>
  );
}
